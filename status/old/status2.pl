#!/usr/bin/perl
# WARNING: This script doesn't work yet.
...
@langs = qw ( bg ca cs da de el en_GB es et eu fi fr ga gl hr hu is it ja ko lt nl no nn pl pt pt_BR ro ru sk sl sp sr sv ta tr uk wa zh_TW.Big5 zh_CN.GB2312 );
...
sub getmerge{
    ...
    open(MERGE,"$cvsroot/$mod/update.pl $lang.po |")
    || die ("unable to update.pl");
    close(MERGE);
}
