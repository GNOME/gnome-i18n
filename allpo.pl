...
#
#    Version 0.3:
#	- when msgstr is empty, no store action is done (this should be an 
#	  optional in the future)
#	- temporary files are used in a safe manner
#	- if commit is added as additional parameter in store line
#	  cvs commit is called for each file when storing it
#
#    Version 0.2: 
#	- added escaping of doublequotes
#	- disabled Slovenian as default language
#	- cleaned up the code a bit
#       - on store each updated file is only printed out once
	...
	$c="\x23";
	@lines;
	%filelist;		# list of all files when storing
...
#	open (OUT, ">$tmpfile");
	...
	do { $tmpfile = tmpnam() }
		until sysopen(OUT, $tmpfile, O_RDWR|O_CREAT|O_EXCL);

	# install atexit-style handler so that when we exit or die,
	# we automatically delete this temporary file
	END { if ($tmpfile) {unlink($tmpfile) or die "Couldn't unlink $tmpfile : $!" }}
...
if (($action eq "load")&&($lang)) {
	...
	print "\n\n$c This file was automatically generated by allpo.pl\n";
	...
	if ($commit eq "commit")  {
		...
		foreach $filename (keys(%filelist)) {
			...
			$path=$1;
			$fn=$2;
			if ($path) {$path="cd $path";} else {$fn=$filename};
			
			print "$path; cvs -z3 commit -m \"Updated $lang translations\" $fn";
			system ("$path; cvs -z3 commit -m \"Updated $lang translations\" $fn");
		}
	}
} else {
...
Usage:
allpo load /directory lang     -- outputs po syntax file to stdout
allpo store file.po            -- takes po file and stores it's translations 
allpo store file.po commit     -- beside storing it allso calls cvs commit
